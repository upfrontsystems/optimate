...
if __name__ == "__main__":
    ...
    if os.name == "posix":
        pathlist = cwd.split("/")
        try: os.remove(('/').join(pathlist[:-5])+'/server.sqlite')
        except OSError, o: pass
    ...
    with transaction.manager:
        ...
        budgetgroupbook = xlrd.open_workbook(exceldatapath+ 'BudgetGroups.xls')
        ...
        for x in range (1, sheet.nrows):
            ...
            if endindex<0: endindex=len(name)-1

            checkname = name[beginindex:endindex].strip()
            resourcequery = DBSession.query(Resource).filter_by(Name=
                                        checkname).first()
